...
%% function
function  ConfigParameters = CohTestConfig()
...
list                                   = {'Kaimal','Mann'};
[index,~]                              = listdlg('PromptString',{'Please select a IEC turbulence model.'},...
    'SelectionMode','single','ListString',{'Kaimal','Mann'});
ConfigParameters.TurbModel             = list{index};


% dialog for the directories 
% 1.TurbSim or Mann turbulence generator, e.g. '..\..\TurbSim_x64.exe' or '..\..\mann_turb_x64.exe' 
% 2.Directory of the input file of TurbSim (.inp) or Mann turbulence generator (batch file)
% 3.Directory to save results. New folders will be created in this directory. 
% by default the new folders will be created in the current drive  

prompt     = {'Enter the directory of executable:','Enter the directory of default input file:'...
              'Enter the directory for results storage:'};
dlgtitle   = 'Directories';
dims       = [1 100];

if strcmp(ConfigParameters.TurbModel,'Kaimal')
    definput = {[pwd '\TurbSim\TurbSim_x64.exe'],[pwd '\example\TurbSimInputFileCohTest.inp'],'G:\test'};
else
    definput = {[pwd '\MannTurb\mann_turb_x64.exe'],[pwd '\example\MannTurbInputFileCohTest.bat'],'G:\test'};
end

answer                                  = inputdlg(prompt,dlgtitle,dims,definput);
ConfigParameters.exeDir                 = answer{1};
ConfigParameters.SimInitialInputDir     = answer{2};
ConfigParameters.saveDir                = answer{3};
...
ConfigParameters.EvoModel        = 'Exp-UserDefined'; % 'Exp-UserDefined', 'Exp-Simley', 'Exp-GPR', 'Kristensen'     
...
if strcmp(ConfigParameters.EvoModel,'Exp-UserDefined')
    ConfigParameters.evo_a             = 2 ;            % Longitudinal coherence decay parameter  
